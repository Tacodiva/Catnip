...
export class IR0InputOperatorAdd extends IR0InputOperatorGenericBinary {
    public constructor(left: IR0Input, right: IR0Input) {
        super("operator_add", CatnipValueFormat.F64_NUMBER, left, right);
    }
    ...
    public emitIR1(emitter: IR1Emitter) {
        return new IR1InstrSimple(this.name, emitter => emitter.emitWasm(SpiderOpcodes.f64_add));
    }

    public clone(ctx: IR0CloneContext) {
        return new IR0InputOperatorAdd(this.args.left.input.clone(ctx), this.args.right.input.clone(ctx));
    }
}
