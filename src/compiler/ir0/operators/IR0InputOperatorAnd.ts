...
export class IR0InputOperatorAnd extends IR0InputOperatorGenericBinary {
    public constructor(left: IR0Input, right: IR0Input) {
        super("operator_and", CatnipValueFormat.I32_BOOLEAN, left, right);
    }
    ...
    public emitIR1(emitter: IR1Emitter) {
        return new IR1InstrSimple(this.name, emitter => emitter.emitWasm(SpiderOpcodes.i32_and));
    }

    public clone(ctx: IR0CloneContext) {
        return new IR0InputOperatorAnd(this.args.left.input.clone(ctx), this.args.right.input.clone(ctx));
    }
