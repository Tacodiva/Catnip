...
export class IR0InputOperatorAtan extends IR0InputOperatorGenericMathop {
    public constructor(input: IR0Input) {
        super("operator_atan", input);
    }

    public emitIR1(emitter: IR1Emitter): IR1Instruction | IR1Instruction[] {
        return new IR1InstrSimple(this.name, emitter => emitter.emitWasmRuntimeFunctionCall("catnip_math_atan", true));
    }

    public clone(ctx: IR0CloneContext): IR0Input<["operand"]> {
        return new IR0InputOperatorAtan(this.args.operand.input.clone(ctx));
    }
}